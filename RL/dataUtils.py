...
def recieveAnswer(user, user_bias, item, item_bias, global_bias, sigma):
    ...
    return gauss(mean, sigma) + user_bias + item_bias + global_bias
...
def OneStep(user, user_bias, item, item_bias, global_bias, r, learning_rate, user_bias_reg, user_fact_reg):
    ...
    e = (r - prediction - SUCSESS())  # error
    user_bias += learning_rate * (e - user_bias_reg * user_bias)

    user += learning_rate * (e * item - user_fact_reg * user)
...
def SortIItemByPopularity(users, users_bias, item, item_bias, global_bias, sigma):
    ...
    for i in range(item.shape[0]):
        result.append([
            sum(float(recieveAnswer(users[j], users_bias[j], item[i], item_bias[i], global_bias, sigma) > SUCSESS) for j in range(users.shape[0])) / n_users , i])
...
def PrintItemPopularity(user_vecs, user_bias, item_vecs, item_bias, global_bias, sigma):
    ...
    with open("itemPopularity", 'w') as ip:
        for i in sort_items:
            ...
            ip.write(str(i[0]) + "\t"  + str(i[1]) + "\t" + items_names[i[1] + 1][0] + '\n')
